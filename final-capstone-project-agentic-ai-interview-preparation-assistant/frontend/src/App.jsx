...
function App() {
  ...
  const [interviewEvaluations, setInterviewEvaluations] = useState(() => {
    try {
      const cached = localStorage.getItem('interview_evaluations');
  ...
  const toggleRecording = () => {
    if (!recognition) {
      alert('Speech Recognition is not supported by your browser. Please use Google Chrome, Safari, or Microsoft Edge.');
      return;
    }
    if (isRecording) {
      recognition.stop();
  ...
  // Stop recording when changing questions or tabs
  useEffect(() => {
    if (isRecording && recognition) {
      recognition.stop();
      setIsRecording(false);
    }
  }, [currentQuestionIdx, dashboardTab]);

  // Readiness Verdict Agent State
  const [username, setUsername] = useState(() => {
    const query = new URLSearchParams(window.location.search);
    return query.get('username') || localStorage.getItem('tv_username') || 'user';
  });
  const [userId, setUserId] = useState(() => {
    const query = new URLSearchParams(window.location.search);
    return query.get('userId') || localStorage.getItem('tv_userid') || '7';
  });
  ...
  const [verdictInputText, setVerdictInputText] = useState('');
  const [verdictActiveTab, setVerdictActiveTab] = useState('auto'); // 'auto' | 'upload' | 'text'

  useEffect(() => {
    localStorage.setItem('tv_username', username);
    localStorage.setItem('tv_userid', userId);
  }, [username, userId]);

  useEffect(() => {
    localStorage.setItem('interview_evaluations', JSON.stringify(interviewEvaluations));
  ...
  const handleAnalyze = async () => {
    ...
    setApiError(null);
    try {
      // Clear previous interview cache and states
      ...
      localStorage.removeItem('interview_evaluations');
  ...
  const handleAutomaticVerdict = async () => {
    setIsEvaluatingVerdict(true);
    setVerdictError(null);
    try {
      const res = await fetch('http://localhost:8002/api/analyze-student', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          username: username,
          interview_history: interviewEvaluations
        })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || 'Readiness evaluation failed');
  ...
  const handleTextVerdict = async () => {
    if (!verdictInputText.trim()) { alert('Please paste the profile text to analyze.'); return; }
    setIsEvaluatingVerdict(true);
    setVerdictError(null);
    try {
      const res = await fetch('http://localhost:8002/api/analyze-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: verdictInputText })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || 'Readiness evaluation failed');
      setVerdictResult(data.verdict);
  ...
  const handleFileVerdict = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8002/api/analyze-file', {
        method: 'POST',
        body: form
      });
      const data = await res.json();
  ...
  return (
    <div className={`relative bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500/30 selection:text-white ${
      currentView === 'dashboard' ? 'h-screen overflow-hidden' : 'min-h-screen'
    }`}>
      ...
      <AnimatePresence mode="wait">
              ...
              <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="px-4 py-2 rounded-xl text-sm font-medium text-slate-300 hover:text-white border border-slate-800 hover:bg-slate-900 transition-all"
                  >
                    ← Back to Profile
                  </button>
                  <button 
        ...
        {currentView === 'dashboard' && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="flex h-screen overflow-hidden z-10 w-full"
          >
            {/* LEFT SIDEBAR - Desktop */}
            <aside className="hidden md:flex flex-col w-64 h-screen border-r border-slate-900/60 bg-slate-950/65 backdrop-blur-md p-6 shrink-0 relative overflow-y-auto">
              ...
              <nav className="space-y-1.5 flex-grow">
                {[
                  ...
                  { id: 'readiness', label: 'Readiness Verdict', icon: TrendingUp },
                ].map((item) => {
            ...
            <main className="flex-grow flex flex-col min-w-0 h-screen overflow-hidden bg-slate-950/40">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-300 hover:text-white border border-slate-850/60 bg-slate-900 hover:bg-slate-800/80 transition-all flex items-center gap-1.5"
                    >
                      ← Exit to Dashboard
                    </button>
              ...
              <div className="flex-grow p-6 overflow-y-auto max-w-7xl w-full mx-auto">
                <AnimatePresence mode="wait">
                            ...
                            <div className="glass-card p-6 rounded-2xl space-y-6 relative overflow-hidden">
                              ...
                              <div className="space-y-3.5">
                                <div className="flex items-center justify-between">
                                  ...
                                  {/* Toggle mode */}
                                  <div className="flex items-center gap-1.5 bg-slate-950/80 p-1 rounded-lg border border-slate-900">
                                    <button
                                      type="button"
                                      onClick={() => {
                                      ...
                                      className={`px-2 py-1 rounded-md text-[9px] font-bold transition-all flex items-center gap-1 ${!isVoiceMode ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                                    >
                                ...
                                {isVoiceMode ? (
                                  <div className="flex flex-col items-center justify-center p-6 min-h-[160px] rounded-xl bg-slate-900/40 border border-slate-800 space-y-4">
                                    {isRecording ? (
                                      <div className="flex flex-col items-center space-y-3">
                                        {/* Wave animation */}
                                        <div className="flex items-center gap-1 h-6">
                                          <div className="w-0.75 bg-indigo-500 rounded-full animate-bounce h-3" style={{ animationDelay: '0.1s' }}></div>
                                          <div className="w-0.75 bg-purple-500 rounded-full animate-bounce h-5" style={{ animationDelay: '0.2s' }}></div>
                                          <div className="w-0.75 bg-pink-500 rounded-full animate-bounce h-4" style={{ animationDelay: '0.3s' }}></div>
                                          <div className="w-0.75 bg-indigo-500 rounded-full animate-bounce h-5" style={{ animationDelay: '0.4s' }}></div>
                                          <div className="w-0.75 bg-purple-500 rounded-full animate-bounce h-2" style={{ animationDelay: '0.5s' }}></div>
                                        </div>
                                        <p className="text-[10px] text-indigo-400 font-bold tracking-wide uppercase animate-pulse">Listening... Speak your answer now</p>
                                      </div>
                                    ) : (
                                      <p className="text-xs text-slate-400 font-medium">Voice Mode Active. Click to start recording.</p>
                                    )}
                                    ...
                                    <button
                                      ...
                                      className={`p-3.5 rounded-full transition-all flex items-center justify-center shadow-lg ${isRecording ? 'bg-red-500 hover:bg-red-600 text-white shadow-red-500/20' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-600/20'}`}
                                    >
                                    ...
                                    {/* Preview container */}
                                    {currentAnswer && (
                                      <div className="w-full mt-2 p-3 rounded-lg bg-slate-950 border border-slate-900 text-left">
                                        <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 font-mono">Transcribed Answer Preview:</div>
                                        <p className="text-xs text-slate-300 leading-relaxed max-h-[100px] overflow-y-auto">{currentAnswer}</p>
                                      </div>
                                    )}
                                  </div>
                                ) : (
                  ...
                  {/* TAB 6: READINESS VERDICT */}
                  {dashboardTab === 'readiness' && (
                    <motion.div 
                      key="readiness"
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-1">
                          <h3 className="text-2xl font-bold tracking-tight text-slate-100">Placement Readiness Verdict</h3>
                          <p className="text-slate-400 text-xs">Evaluates overall readiness using sandbox mock results, projects portfolio, and live coding records.</p>
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-3 gap-8">
                        ...
                        <div className="md:col-span-1 space-y-6">
                          ...
                          {/* Automatic verdict option */}
                          <div className="glass-card p-6 rounded-2xl space-y-4">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Automatic Sandbox Analysis</h4>
                            <p className="text-xs text-slate-400 leading-normal">
                              Evaluates your database profile handles alongside current Sandbox mock round evaluations.
                            </p>
                            
                            <div className="p-3 rounded-xl bg-slate-900/30 border border-slate-900 text-xs">
                              <span className="font-semibold block text-slate-200">Logged Student:</span>
                              <span className="text-indigo-400 font-bold block mt-0.5">@{username}</span>
                              <span className="text-[10px] text-slate-500 block mt-2">Sandbox Status:</span>
                              {interviewEvaluations.length > 0 ? (
                                <span className="text-emerald-400 font-bold block">✓ Completed ({interviewEvaluations.length} Questions)</span>
                              ) : (
                                <span className="text-amber-500 font-bold block">⚠ Sandbox Pending</span>
                              )}
                            </div>

                            <button
                              onClick={handleAutomaticVerdict}
                              disabled={isEvaluatingVerdict || interviewEvaluations.length === 0}
                              className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:hover:bg-indigo-600 text-xs font-bold text-white shadow-md flex items-center justify-center gap-2"
                            >
                              {isEvaluatingVerdict && verdictActiveTab === 'auto' ? (
                                <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Evaluating...</>
                              ) : (
                                <><Sparkles className="w-3.5 h-3.5" /> Analyze Sandbox Profile</>
                              )}
                            </button>
                          </div>

                          {/* Manual Upload & Paste Options */}
                          <div className="glass-card p-6 rounded-2xl space-y-4">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Manual Document Analysis</h4>
                            
                            {/* Tab toggles */}
                            <div className="grid grid-cols-2 gap-2 bg-slate-900/40 p-1 rounded-xl">
                              <button 
                                onClick={() => setVerdictActiveTab('upload')}
                                className={`py-1.5 text-[10px] font-bold rounded-lg transition-colors ${verdictActiveTab === 'upload' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                              >
                                File Upload
                              </button>
                              <button 
                                onClick={() => setVerdictActiveTab('text')}
                                className={`py-1.5 text-[10px] font-bold rounded-lg transition-colors ${verdictActiveTab === 'text' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                              >
                                Paste Profile
                              </button>
                            </div>

                            {verdictActiveTab === 'upload' && (
                              <div className="space-y-4">
                                <div className="border border-dashed border-slate-800 rounded-xl p-4 text-center cursor-pointer hover:border-slate-705 transition-colors relative">
                                  <input 
                                    type="file" 
                                    accept=".txt,.md"
                                    onChange={handleFileVerdict}
                                    className="absolute inset-0 opacity-0 cursor-pointer"
                                    disabled={isEvaluatingVerdict}
                                  />
                                  <UploadCloud className="w-8 h-8 text-slate-500 mx-auto mb-2" />
                                  <span className="text-[10px] font-semibold text-slate-400 block">Click to upload TXT or MD</span>
                                  <span className="text-[8px] text-slate-500 block mt-1">Accepts custom text resumes or markdown profiles</span>
                                </div>
                              </div>
                            )}

                            {verdictActiveTab === 'text' && (
                              <div className="space-y-3">
                                <textarea
                                  placeholder="Paste custom candidate metrics or markdown resume details here..."
                                  value={verdictInputText}
                                  onChange={(e) => setVerdictInputText(e.target.value)}
                                  className="w-full h-24 bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-slate-300 placeholder-slate-500 focus:outline-none focus:border-indigo-600 resize-none"
                                  disabled={isEvaluatingVerdict}
                                />
                                <button
                                  onClick={handleTextVerdict}
                                  disabled={isEvaluatingVerdict || !verdictInputText.trim()}
                                  className="w-full py-2 text-xs font-bold bg-slate-900 hover:bg-slate-850 border border-slate-800 text-indigo-400 rounded-xl transition-all"
                                >
                                  {isEvaluatingVerdict && verdictActiveTab === 'text' ? 'Analyzing...' : 'Analyze Text'}
                                </button>
                              </div>
                            )}
                          </div>
                        ...
                        {/* RIGHT COLUMN: EVALUATION RESULT VIEW */}
                        <div className="md:col-span-2 space-y-6">
                          {isEvaluatingVerdict ? (
                            <div className="glass-card p-12 rounded-2xl text-center space-y-4">
                              <RefreshCw className="w-8 h-8 text-indigo-400 animate-spin mx-auto" />
                              <h4 className="font-bold text-sm">Verdict Agent Processing...</h4>
                              <p className="text-xs text-slate-400 max-w-sm mx-auto">
                                The agent is recursively querying student records, parsing domain projects, and reasoning mock interview transcripts to compute a comprehensive placement suitability scorecard.
                              </p>
                            </div>
                          ) : verdictError ? (
                            <div className="glass-card p-8 rounded-2xl border-rose-500/20 text-center space-y-3">
                              <AlertCircle className="w-8 h-8 text-rose-400 mx-auto" />
                              <h4 className="font-bold text-rose-400 text-sm">Analysis Failed</h4>
                              <p className="text-xs text-slate-400">{verdictError}</p>
                            </div>
                          ) : verdictResult ? (
                            <div className="space-y-6">
                              
                              {/* Top Verdict KPI Summary */}
                              <div className="glass-card p-6 rounded-2xl space-y-4">
                                <div className="flex justify-between items-center border-b border-slate-900 pb-3">
                                  <h4 className="text-xs font-bold text-slate-200 uppercase tracking-widest flex items-center gap-2">
                                    <Award className="w-4.5 h-4.5 text-indigo-400" /> Evaluation Summary
                                  </h4>
                                  <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                                    verdictResult.bottleneck.toLowerCase().includes('no significant bottleneck') 
                                      ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' 
                                      : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                                  }`}>
                                    {verdictResult.bottleneck.toLowerCase().includes('no significant bottleneck') ? 'Suitable' : 'Needs Work'}
                                  </span>
                                </div>

                                <div className="space-y-4">
                                  <div>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Activity Check</span>
                                    <p className="text-xs text-slate-200 font-semibold mt-1">{verdictResult.activity_check}</p>
                                  </div>

                                  <div className="p-4 rounded-xl bg-slate-900/40 border border-slate-900 space-y-1">
                                    <span className="text-[10px] font-bold text-rose-400 uppercase tracking-wider">Identified Bottleneck</span>
                                    <p className="text-xs text-slate-200 font-bold leading-relaxed">{verdictResult.bottleneck}</p>
                                  </div>

                                  <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/10 space-y-1">
                                    <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider">Next Action</span>
                                    <p className="text-xs text-slate-200 font-bold leading-relaxed">{verdictResult.next_action}</p>
                                  </div>

                                  <div>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Agent Reasoning</span>
                                    <p className="text-xs text-slate-350 leading-relaxed mt-1.5">{verdictResult.reasoning}</p>
                                  </div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="glass-card p-12 rounded-2xl text-center space-y-4">
                              <Brain className="w-10 h-10 text-indigo-400/80 mx-auto" />
                              <h4 className="font-bold text-base text-slate-200">Awaiting Analysis Trigger</h4>
                              <p className="text-xs text-slate-400 max-w-sm mx-auto">
                                Run the automatic verdict using your current Sandbox mock interview history, or manually paste/upload your capability documentation in the left panel to begin.
                              </p>
                            </div>
                          )}
                        </div>
